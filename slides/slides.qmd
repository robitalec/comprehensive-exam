...
Examples of payoffs are an amount of a particular resource as well as predator dilution, thermal insulation. Examples of penalties are competition and contests with conspecifics

A typical iterative model where individuals play a strategy at each time step like the Barnard and Sibly 1981 model have the following structure:

Producer's find patches given some patch discovery rate and have sole access to a portion of the patch, a finder's share, before scroungers arrive. Scroungers divide all discovered patches in a time step, minus the finder's share, equally.
...
::: {.notes}
The second goal was to determine how access to resources is modified by risks and conditions
...
We'll look at the results in the following slides, and I've included some examples of other resources, risks and conditions that might be important to consider but weren't addressed in the literature, indicated with a count of 0
:::
...
and patch quality included patch richness and patch depletability

Some influences of resources on foraging strategy include that
...
There was only one empirical test of predation risk. Bugnyar and Kotrschal observed ravens foraging either with wolves, where they assumed the predation risk was high, or wild boars, where they assumed the predation risk was low. They found that ravens were more likely to scrounge when the predation risk was high.  

However, their definitions don't directly relate to classic definitions of producing and scrounging. They defined producing as stealing from allospecifics and scrounging as stealing from conspecifics. Still a very interesting experiement incorporating interspecific relationships. 
:::
...
So in total, the producer-scrounger literature hasn't really incorporated habitat variables that I would consider risks and conditions. This is likely to due how these data are collected. Lots of the empirical tests are controlled experiments that are looking for answers to different questions than how habitat influences producer-scrounger dynamics. 

I did, however, find a ton of other variables that are important in these systems. 
:::
...
::: {.notes}
Mostly so I could make a joke where I wave my hands at it and say don't look too close

But really we want to look closely at this

There are a ton variables that I gleaned from the producer-scrounger literature that could have a impact on habitat selection

Ony really strong one is dominance. Dominance makes a huge difference in producer-scrounger dynamics because a dominant individual is more likely to scrounge because it can just bully others of their patches

Another is relatedness, where producers are more likely to share with related scroungers 

And this leads into a bonus objective
:::
...
::: {.column width="35%"}
1. Represent system using a DAG
1. Determine if effects are identificable, or confounded
:::
::::

::: {.notes}
This is a bonus objective, but could have also been an interesting comprehensive exam question in and of itself:

How has ignoring causal inference done a disservice to habitat selection analyses?

Habitat selection analyses have three different objectives: measuring species distribution, inferring why animals use certain habitats and predicting how use would change in new habitats or across time

One large issue in the habitat selection literature is the use of predictive models for causal inference 

There are published articles recommending ranking habitat selection models based on predictive score (AIC) then discussions that follow treating these top models as models for inference.

The trouble is predictive score is often improved by confounded models and typically by models with more covariates. 

A principled solution to avoiding confounded models is to first draw a DAG (directed acyclic graph) with your knowledge of the system and not the relationships you try to glean from the data

Then determine if your total and direct effects are identifiable given the system

On the left we have an example for a hypothetical ecological system from a paper by Arif and MacNeil. You can read these graphs as, for example, fire is some function of climate. Interestingly, we can see that climate, for example, has multiple pathways to the variable Species Y indirectly through Forestry and directly. 

I'd love dig into this more, but obviously it's a bonus and sort of tangential to the question
...
Scroungers, if they are purely playing the scrounger behaviour, only have what producers make available for them. Therefore, the density is fixed around the distance to their producer conspecifics (A, B, C). 
...
I took the producer step length and turn angle distributions and biased them by the location of conspecifics. This now represents how an individual could consider switching strategies to either follow a conspecific as a scrounger, or to independently forage as a producer. 

To calculate these for each movement step would require measuring the distances between the focal individual and its conspecifics to bias the availability distributions. I also assigned less weight to further neighbours because I assumed that individuals were more likely to scrounge from nearby neighbours.
:::


## <!-- Gaps and future directions -->  {.center}
Objectives

1. Access depending on foraging strategy

2. Risks and conditions influence on strategies

3. Availability integrating foraging strategy
