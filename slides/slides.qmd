...
## <!-- Disproportionate effect -->{.center}
...
consider how behavioural strategies influence availability andd access
-->
...
## <!-- Objective 3: Distributions --> {.center} 
...
prod_sl <- data.frame(x = rgamma(1e3, 3, 2)) |>
	...
	labs(x = 'step length', y = 'density', title = 'Producer') + 
	theme_minimal(base_size = 30)
...
scr_sl <- data.frame(x = c(
	...
	geom_label(data = data.frame(x = c(2.5, 3.2, 4), y = rep(0.4, 3), id = LETTERS[seq.int(3)]), aes(x, y, label = id)) + 
 	labs(x = 'step length', y = 'density', title = 'Scrounger') + 
	theme_minimal(base_size = 30)
